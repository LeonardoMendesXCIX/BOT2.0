...
exports.TOTAL_FUNNY = void 0;
exports.getFunnyMessage = getFunnyMessage;
...
exports.TOTAL_FUNNY = ABERTURAS.length * CENARIOS.length * FINAIS.length;
function getFunnyMessage(storage) {
    if (!storage.data.cache)
        storage.data.cache = {};
    if (!Array.isArray(storage.data.cache.funnyUsed))
        storage.data.cache.funnyUsed = [];
    if (storage.data.cache.funnyUsed.length >= exports.TOTAL_FUNNY)
        storage.data.cache.funnyUsed = [];
    let idx = Math.floor(Math.random() * exports.TOTAL_FUNNY);
    let guard = 0;
    while (storage.data.cache.funnyUsed.includes(idx) && guard < 60) {
        idx = Math.floor(Math.random() * exports.TOTAL_FUNNY);
        guard++;
    }
