...
export async function callAI(prompt: string, contextCluster: string[] = []): Promise<string> {
    if (!SETTINGS.GROQ_API_KEY) return "⚠️ Erro de Configuração: Chave Groq API não configurada.";
...
export async function evaluateAutonomousIntervention(contextCluster: string[]): Promise<string | null> {
    if (!SETTINGS.GROQ_API_KEY || !contextCluster || contextCluster.length === 0) return null;
    const url = 'https://api.groq.com/openai/v1/chat/completions';
    ...
    try {
        ...
        if (out === 'SILENCE' || out.startsWith('SILENCE') || out.length < 5) return null;
        return out;
    } catch (e) {
        return null;
    }
