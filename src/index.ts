...
process.on("unhandledRejection", (reason: any) => {
  ...
  console.log(
    "[SISTEMA] Rejeição assíncrona interceptada:",
...
cron.schedule(
  ...
  async () => {
    ...
    for (let i = 0; i < storage.data.scheduledMsgs.length; i++) {
      ...
      if (
        storage.isBotDisabled(sched.chatId) ||
      ...
      for (const chatId in storage.data.groupSchedules) {
        ...
        if (sched.openTime === currentHHMM) {
          try {
            ...
            console.log(
              "[AGENDA] Grupo " + chatId + " aberto às " + currentHHMM,
            );
          } catch (e: any) {
        ...
        if (sched.closeTime === currentHHMM) {
          try {
            ...
            console.log(
              "[AGENDA] Grupo " + chatId + " fechado às " + currentHHMM,
            );
          } catch (e: any) {
...
setInterval(() => {
  if (lastDisconnectAt > 0) {
    ...
    if (downMs >= 5 * 60 * 1000 && !watchdogNotified) {
      ...
      console.log(
        "[WATCHDOG] ⚠️ Bot fora do ar há mais de 5 minutos (desde " +
          downStartStr +
  ...
  try {
    ...
    for (const chatId in storage.data.groupSchedules) {
      ...
      if (
        ...
        continue;
      const shouldOpen = isWithinWindow(
        sched.openTime,
        sched.closeTime,
        currentHHMM,
      );
      ...
      if (shouldOpen && isAnnouncement) {
        ...
        storage.setGroupClosed(chatId, false);
        console.log(
          "[AGENDA BOOT] Grupo " + chatId + " deveria estar ABERTO → aberto.",
        );
      } else if (!shouldOpen && !isAnnouncement && sched.closeTime) {
        await sock.groupSettingUpdate(chatId, "announcement");
        storage.setGroupClosed(chatId, true);
        console.log(
          "[AGENDA BOOT] Grupo " + chatId + " deveria estar FECHADO → fechado.",
        );
      }
    }
  } catch (e: any) {
        ...
        try {
          ...
          if (bvConfig && bvConfig.text) {
            ...
            if (userText.includes("{membro}")) {
              ...
              userText = userText.replace(
                /\{nome\}/gi,
                memberInfo.pushName || memberInfo.formattedNum,
              );
  ...
  setInterval(async () => {
    ...
    for (const chatId in storage.data.autoAnim) {
      if (storage.isBotDisabled(chatId) || storage.isGroupClosed(chatId))
        continue;
      if (
        storage.data.autoAnim[chatId] === true &&
